...
export const MyLeaveRequestCard: React.FC<MyLeaveRequestCardProps> = ({ req, onSelectDoctorDetails }) => {
  ...
  const { doctors, getOrCreateChatRoom, user, getCoverageFeedback } = useData();
  ...
  useEffect(() => {
    if (isRequester && isDutyCompleted && req.hasFeedback) {
      getCoverageFeedback(req.id).then((data) => {
        if (data) setFeedbackData(data);
      });
    }
  }, [isRequester, isDutyCompleted, req.hasFeedback, req.id, getCoverageFeedback]);
          ...
          {!req.hasFeedback && !feedbackData ? (
            ...
            <div className="mt-2 p-5 rounded-xl border border-slate-200 bg-slate-50 flex flex-col gap-3 animate-in fade-in">
              ...
              <div>
                ...
                <p className="text-sm text-slate-700">"{feedbackData.feedback}"</p>
              </div>
            </div>
          ) : null}
        </>
