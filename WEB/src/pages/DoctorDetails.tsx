...
export const DoctorDetails: React.FC<DoctorDetailsProps> = ({
  ...
  const { user } = useAuth();
  const { leaveRequests, getDoctorFeedbackList } = useData();
  ...
  React.useEffect(() => {
    if (doctor) {
      ...
      const fetchFbs = async () => {
        const fbList = await getDoctorFeedbackList(doctor.id || doctor.uid || "");
        setFeedbacks(fbList);
      };
      fetchFbs();
      return;
    ...
    const fetchDoctor = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const fbList = await getDoctorFeedbackList(id);
          setFeedbacks(fbList);
        } else {
          setError("Doctor profile not found.");
        }
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                {localDoctor.coverageRatingCount ? (
                  <span className="flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-800 text-xs font-bold rounded-lg border border-amber-200/60">
                    ...
                    {localDoctor.coverageRating?.toFixed(1) || "0.0"} / 5
                    <span className="text-amber-700/60 font-medium ml-1">({localDoctor.coverageRatingCount} Reviews)</span>
                  </span>
                ) : null}
              </div>
            </div>
