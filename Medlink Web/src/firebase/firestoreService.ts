...
function mapDocToUser(id: string, data: any): User {
  ...
  return {
    ...
    sentimentScoreSum: typeof data.sentimentScoreSum === "number" ? data.sentimentScoreSum : 0,
    sentimentScoreCount: typeof data.sentimentScoreCount === "number" ? data.sentimentScoreCount : 0,
  };
}
...
export async function submitCoverageFeedback(feedback: CoverageFeedback): Promise<void> {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (userSnap.exists()) {
        const userData = userSnap.data();
        currentSentimentSum = typeof userData.sentimentScoreSum === "number" ? userData.sentimentScoreSum : 0;
        currentSentimentCount = typeof userData.sentimentScoreCount === "number" ? userData.sentimentScoreCount : 0;
      }
      if (typeof feedback.sentimentScore === "number") {
        currentSentimentSum += feedback.sentimentScore;
        currentSentimentCount += 1;
      }
      const newCoverageScore = currentSentimentCount > 0 ? Math.round(currentSentimentSum / currentSentimentCount) : 100;
      ...
      transaction.update(userRef, {
        ...
        sentimentScoreSum: currentSentimentSum,
        sentimentScoreCount: currentSentimentCount,
...
export async function updateFeedbackSentiment(
  ...
  try {
    const fbRef = doc(db, "coverageFeedback", feedbackId);
    await updateDoc(fbRef, {
      sentiment,
      sentimentScore: score
    });
  } catch (err) {
    console.error("updateFeedbackSentiment error:", err);
    throw err;
  }
}
  ...
  try {
    const q = query(
      ...
      or(
        ...
        where("reviewedDoctorId", "==", doctorId)
      )
    );
    const snaps = await getDocs(q);
...
export function subscribeDoctorFeedbacks(doctorId: string, onData: (feedbacks: CoverageFeedback[]) => void): () => void {
  try {
    const q = query(
      collection(db, "coverageFeedback"),
      or(
    ...
    return onSnapshot(
      q,
      (snapshot) => {
        const feedbacks = snapshot.docs.map(docSnap => docSnap.data() as CoverageFeedback);
        feedbacks.sort((a, b) => b.createdAt - a.createdAt);
        onData(feedbacks);
      },
      (err) => {
        console.warn("subscribeDoctorFeedbacks error:", err.message);
        onData([]);
      }
    );
  } catch (err) {
    console.error("subscribeDoctorFeedbacks setup error:", err);
    onData([]);
    return () => {};
  }
}
...
export function subscribeCoverageFeedback(requestId: string, onData: (feedback: CoverageFeedback | null) => void): () => void {
  try {
    const fbRef = doc(db, "coverageFeedback", requestId);
    return onSnapshot(
      fbRef,
      (docSnap) => {
        if (docSnap.exists()) {
          onData(docSnap.data() as CoverageFeedback);
        } else {
          onData(null);
        }
      },
      (err) => {
        console.warn("subscribeCoverageFeedback error:", err.message);
        onData(null);
      }
    );
  } catch (err) {
    console.error("subscribeCoverageFeedback setup error:", err);
    onData(null);
    return () => {};
  }
}
