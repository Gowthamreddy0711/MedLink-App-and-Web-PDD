...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const myRequests = leaveRequests.filter((r) => r.doctorId === user.id || r.requesterUid === user.id || (user.email && (r.doctorEmail === user.email || (r as any).email === user.email)));
  ...
  const myCurrentRequests = myRequests.filter(r => !isHistorical(r));
  const myPendingRequests = myCurrentRequests.filter(r => r.status === "PENDING" || r.status === "OPEN" || r.status === "Open").length;
  const myApprovedRequests = myCurrentRequests.filter(r => r.status === "APPROVED" || r.status === "ACCEPTED" || r.status === "Assigned").length;
  const myCompletedRequestsCount = myRequests.filter(r => isHistorical(r)).length;
  ...
  const myDuties = leaveRequests.filter((r) => r.approvedDoctorId === user.id || r.assignedVolunteerUid === user.id || (user.email && (r.approvedDoctorEmail === user.email || (r as any).volunteerEmail === user.email)));
  const myActiveDutiesCount = myDuties.filter(r => !isHistorical(r)).length;
  const myCompletedDutiesCount = myDuties.filter(r => isHistorical(r)).length;
