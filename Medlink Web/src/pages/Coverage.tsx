...
export const Coverage: React.FC<CoverageProps> = ({ onSelectDoctorDetails }) => {
  ...
  const myRequests = useMemo(() => leaveRequests.filter((r) => r.doctorId === user.id || r.requesterUid === user.id), [leaveRequests, user.id]);
  ...
  const myDuties = useMemo(() => leaveRequests.filter((r) => r.approvedDoctorId === user.id || r.assignedVolunteerUid === user.id), [leaveRequests, user.id]);

  const myCurrentRequests = useMemo(() => myRequests.filter(r => r.status !== "COMPLETED").sort((a, b) => b.createdAt - a.createdAt), [myRequests]);
  const myCompletedRequests = useMemo(() => myRequests.filter(r => r.status === "COMPLETED").sort((a, b) => b.createdAt - a.createdAt), [myRequests]);

  const myActiveDuties = useMemo(() => myDuties.filter(r => r.status !== "COMPLETED").sort((a, b) => b.createdAt - a.createdAt), [myDuties]);
  const myCompletedDuties = useMemo(() => myDuties.filter(r => r.status === "COMPLETED").sort((a, b) => b.createdAt - a.createdAt), [myDuties]);
